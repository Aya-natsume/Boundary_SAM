...
def attach_boundary_point_features(
    boundary_dict: Dict[BoundaryKey, Dict[str, torch.Tensor]],
    feature_map: torch.Tensor,
) -> Dict[BoundaryKey, Dict[str, Union[int, torch.Tensor, Tuple[int, int]]]]:
    """
    为每个边界类别补充对应点位的原始特征与归一化特征。

    这个函数的作用很直接：
        1. `assign_fine_boundary_labels` 只负责几何归类，不负责特征
        2. 后续无论是特征一致性筛选，还是过滤前后的特征可视化，都需要先把点特征挂上去
        3. 这里单独拆出来，后面训练接入时会更灵活，不会把几何逻辑和特征逻辑拧成一团

    参数:
        boundary_dict: dict
            细粒度边界字典，通常来自 `assign_fine_boundary_labels`。
        feature_map: torch.Tensor
            特征图，形状为 `(B, C, H, W)`。

    返回:
        enriched_boundary_dict: dict
            在原字典基础上新增：
            - `features`: `(N, C)` 原始特征
            - `features_norm`: `(N, C)` L2 归一化特征
    """
    if feature_map.ndim != 4:
        raise ValueError(f"feature_map 必须是 4 维张量，当前 shape={tuple(feature_map.shape)}")

    enriched_boundary_dict = {}
    for pair_key, boundary_info in boundary_dict.items():
        ...
        # 这里做浅拷贝就够了，原始坐标张量不需要重复复制一大份。
        enriched_info = dict(boundary_info)
        enriched_info["features"] = raw_features
        enriched_info["features_norm"] = normalized_features
        enriched_boundary_dict[pair_key] = enriched_info

    return enriched_boundary_dict
...
def filter_boundary_points_by_feature_consistency(
    ...
    for pair_key, boundary_info in boundary_dict.items():
        ...
        # 如果上游已经挂好了特征，就直接复用，别重复 gather。
        if "features" in boundary_info and "features_norm" in boundary_info:
            raw_features = boundary_info["features"].to(torch.float32)
            normalized_features = boundary_info["features_norm"].to(torch.float32)
        else:
            # 否则就在这里补取一次，保证函数本身可以独立使用。
            raw_features = gather_point_features(feature_map, coords).to(torch.float32)
            normalized_features = F.normalize(raw_features, p=2, dim=1, eps=1e-12)
        ...
        filtered_boundary_dict[pair_key] = {
            ...
            "raw_features_norm": normalized_features,
...
def build_source_fine_boundary_points(
    ...
    # 先把原始几何候选点对应的特征挂上去，后面筛选和特征可视化都直接复用这一份。
    raw_boundary_dict = attach_boundary_point_features(
